...
import viewPatientIcon from "../../../assets/patient.png"; // Import the patient view icon
...
const AdminDashboard = () => {
    ...
    const isVerifiedAdmin = async () => {
      try {
        ...
        const authResponse = await axios.post(
          "http://localhost:3001/admins/verified",
          {},
  ...
  const fetchPatients = async () => {
    try {
      const response = await axios.get(
        "http://localhost:3001/admins/patients",
        {
          withCredentials: true,
        }
      );
  ...
  const handlePatientEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const { userId, email, password, doctorId, hospital_id, patientDetails } =
        editingPatient;
      ...
      const response = await axios.put(
        "http://localhost:3001/admins/update-patient",
        updatedPatient,
        {
          withCredentials: true,
        }
  ...
  const handleHospitalEditSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        "http://localhost:3001/admins/update-hospital",
        updatedHospital,
        {
          withCredentials: true,
        }
      ...
      setHospitals(
        hospitals.map((hospital) =>
          hospital._id === editingHospital._id ? editingHospital : hospital
        )
      );
        ...
        <div className="content-wrapper">
          {!view && (
            <div className="white-container">
              ...
              <div
                className={`buttons-container ${isInitial ? "large" : "small"}`}
              >
                <button
                  className={`card-button ${isInitial ? "large" : "small"}`}
  ...
  <>
    ...
    <div className="navigation-buttons">
      <button
        className="small-navigation-button"
        onClick={handleViewHospitals}
      >
        View Hospitals
      </button>
    ...
    <div className="form-actions">
      ...
      <button type="button" onClick={() => setEditingPatient(null)}>
        Cancel
            ...
            {editingHospital && (
              <form
                className="edit-hospital-form"
                onSubmit={handleHospitalEditSubmit}
              >
                <h2>Edit Hospital</h2>
                <label>
                  Hospital Name:
